...
FONT_DIR="/usr/share/fonts/MapleMono"
VERSION_FILE="$FONT_DIR/.version"
CHECK_FILE="MapleMono-CN-unhinted.check"
TEMP_DIR=""
...
cleanup() {
  ...
  rm -f "$FONT_ZIP" "$CHECKSUM_FILE" "$CHECK_FILE" 2>/dev/null || true
  exit $exit_code
}
...
main() {
  ...
  if [ -z "$CHECKSUM_URL" ]; then
    style_error "Error: Could not find checksum file in latest release"
    exit 1
  fi
  ...
  spinner_run "Downloading checksum files..." curl -fsSL -O "$CHECKSUM_URL"
  ...
  # Verify checksum
  style_info "Verifying checksum..."
  HASH=$(tr -d '\n' <"$CHECKSUM_FILE")
  echo "$HASH  $FONT_ZIP" >"$CHECK_FILE"

  if sha256sum -c "$CHECK_FILE" >/dev/null 2>&1; then
    style_success "✓ Checksum verified"
  else
    style_error "✗ Checksum verification failed"
    exit 1
  fi
